...
ensure_shell_registered() {
  ...
  if [[ "${JSH_CONFIGURE_DRY_RUN:-0}" == 1 || "${JSH_INSTALL_DRY_RUN:-0}" == 1 ]]; then
    jsh::log_detail "Would register ${target_shell} in ${shells_file}"
    return 0
  fi

  if declare -F jsh_run_root > /dev/null 2>&1; then
    jsh::log_detail "Registering ${target_shell} in ${shells_file}..."
...
change_shell() {
  ...
  if [[ "${JSH_CONFIGURE_DRY_RUN:-0}" == 1 || "${JSH_INSTALL_DRY_RUN:-0}" == 1 ]]; then
    jsh::log_detail "Would change default shell for ${user_name} to ${target_shell}"
    return 0
  fi
  ...
  jsh::log_error "Unable to change default shell: neither chsh nor usermod succeeded."
  return 1
}
...
main() {
  ...
  if [[ "${current_shell##*/}" == "zsh" ]]; then
    jsh::log_note "Default shell is already Zsh (${current_shell})."
    return 0
  fi

  target_shell=$(find_target_shell || true)
  if [[ -z "${target_shell}" ]]; then
    jsh::log_error "Zsh is unavailable; install it before configuring the default shell."
    return 1
  fi

  jsh::log_detail "Current login shell for ${user_name} is ${current_shell:-unknown}."
  if ! jsh::confirm "Change default shell to Zsh (${target_shell})?" --default yes; then
    jsh::log_note "Skipping default shell change."
    return 0
  fi
  ...
  if change_shell "${target_shell}" "${user_name}"; then
    ...
    if [[ "${new_shell##*/}" == "zsh" ]]; then
      jsh::log_success "Default shell changed to Zsh (${new_shell})."
      jsh::log_note "You may need to log out and log back in for the change to take effect in new sessions."
    elif [[ "${JSH_CONFIGURE_DRY_RUN:-0}" == 1 || "${JSH_INSTALL_DRY_RUN:-0}" == 1 ]]; then
      jsh::log_success "Dry run: would change default shell to Zsh (${target_shell})."
    else
      jsh::log_error "Shell change command completed, but default shell is currently ${new_shell}."
      return 1
    fi
  else
    jsh::log_error "Failed to change default shell to ${target_shell}."
    return 1
  fi
}
