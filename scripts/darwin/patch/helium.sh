#!/usr/bin/env bash
# Upgrade and harden Helium on macOS.
...
readonly POLICY_PATH="/Library/Managed Preferences/${POLICY_USER}/${EXPECTED_BUNDLE_ID}.plist"
...
chromium_milestone() {
  "$(app_executable)" --version 2>/dev/null | /usr/bin/sed -E 's/.*Chromium[[:space:]]+([0-9]+).*/\1/'
}

require_stopped() {
  /usr/bin/pgrep -f "^${APP_PATH}/Contents/MacOS/" >/dev/null 2>&1 && die 1 "Quit Helium first."
  return 0
}

stop_for_apply() {
  local authorized="${1:-false}" answer attempt
  /usr/bin/pgrep -f "^${APP_PATH}/Contents/MacOS/" >/dev/null 2>&1 || return 0
...
upgrade_app() {
  ...
  [[ "$(/usr/bin/uname -s)" == "Darwin" ]] || die 1 "This script supports macOS only."
...
verify_app() {
  ...
  local bundle_id team_id milestone
...
verify_extension_policy() {
  ...
  /usr/bin/osascript -l JavaScript "${PROFILE_HELPER}" verify-policy \
...
configure_extensions() {
  ...
  for ((attempt = 0; attempt < 100; attempt++)); do
    /usr/bin/pgrep -f "^${APP_PATH}/Contents/MacOS/" >/dev/null 2>&1 || break
    /bin/sleep 0.1
  done
  ...
  /usr/bin/osascript -l JavaScript "${PROFILE_HELPER}" pin-extensions \
...
stage_json() {
  ...
  /usr/bin/osascript -l JavaScript "${PROFILE_HELPER}" stage \
...
stage_history() {
  ...
  /usr/bin/sqlite3 "${staged_history}" <<'SQL'
CREATE TABLE urls(
...
verify_json() {
  local preferences_file="$1"
  local local_state_file="$2"
  /usr/bin/osascript -l JavaScript "${PROFILE_HELPER}" verify \
...
verify_history() {
  ...
  result="$(/usr/bin/sqlite3 -batch -readonly "${history_file}" \
    'PRAGMA integrity_check; SELECT url, visit_count, typed_count, hidden FROM urls ORDER BY id;')" ||
    die 1 "Cannot verify the History database."
  [[ "${result}" == $'ok\nhttp://go/|0|1|0' ]] || die 1 "The go host classifier is missing or browsing history remains. Run apply again."
}
