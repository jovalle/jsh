...
install_native_packages() {
  ...
  if ((${#unavailable[@]} > 0)); then
    jsh::log_error "Unresolved ${DISTRO_FAMILY} packages: ${unavailable[*]}"
    return 1
  fi
  if ((${#missing[@]} > 0)); then
    if [[ "${DRY_RUN}" == 1 ]]; then
      jsh::log_detail "Would install native packages: ${missing[*]}"
    else
      case "${DISTRO_FAMILY}" in
  ...
  if [[ "${DISTRO_FAMILY}" == debian && "$(jsh_linux_desktop)" == xfce ]] && package_installed xfce4-screensaver && package_installed light-locker; then
    if [[ "${DRY_RUN}" == 1 ]]; then
      jsh::log_detail "Would remove light-locker (breaks display wakeup; replaced by xfce4-screensaver)."
    else
  ...
  if [[ ${DRY_RUN} != 1 ]]; then
    for package in "${NATIVE_PACKAGES[@]}"; do
      package_installed "${package}" || {
        jsh::log_error "Package verification failed: ${package}"
        return 1
      }
    done
    if ((${#missing[@]})); then
      jsh::log_success "Native ${DISTRO_FAMILY} packages are installed."
    else
      jsh::log_note "Native ${DISTRO_FAMILY} packages are current."
    fi
  fi
}
...
update_native_packages() {
  if [[ "${DRY_RUN}" == 1 ]]; then
    jsh::log_detail "Would update native ${DISTRO_FAMILY} packages."
    return
  fi
  case "${DISTRO_FAMILY}" in
  ...
  jsh::log_success "Native ${DISTRO_FAMILY} packages are up to date."
}
...
prepare_native_packages() {
  ...
  if [[ ${JSH_UPGRADE_SYSTEM:-0} == 1 || "${DISTRO_FAMILY}" == arch ]]; then
    ...
    if [[ "${DRY_RUN}" == 1 ]]; then
      jsh::log_detail "Would refresh Debian package metadata."
    else
      jsh_run_root apt-get update
    fi
  fi
...
install_flatpaks() {
  ...
  if ((${#missing[@]} == 0)); then
    jsh::log_note "Flatpak applications are current."
    return
  fi
  if [[ "${DRY_RUN}" == 1 ]]; then
    jsh::log_detail "Would configure Flathub and install: ${missing[*]}"
    return
  fi
  ...
  for application in "${FLATPAK_APPLICATIONS[@]}"; do
    flatpak info --user "${application}" > /dev/null 2>&1 || {
      jsh::log_error "Flatpak verification failed: ${application}"
      return 1
    }
  done
  jsh::log_success "Flatpak applications are installed."
}

update_flatpaks() {
  command -v flatpak > /dev/null 2>&1 || return
  if [[ "${DRY_RUN}" == 1 ]]; then
    jsh::log_detail "Would update user Flatpak applications."
    return
  fi
  flatpak update --user --noninteractive
  jsh::log_success "Flatpak applications are up to date."
}
...
main() {
  ...
  select_native_packages || {
    jsh::log_note "No native package map for this Linux distribution; using portable installers."
    return
  }

  command -v "${PACKAGE_MANAGER}" > /dev/null 2>&1 || {
    jsh::log_error "${PACKAGE_MANAGER} is required on ${DISTRO_FAMILY}-family systems."
    return 1
  }
  jsh::confirm "Install native ${DISTRO_FAMILY} and Flatpak packages?" --default yes || {
    jsh::log_note "Skipping native packages."
    return 0
  }
